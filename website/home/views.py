...
def index(request):
    ...
    tags = make_tags(get_tag_counts("Somewhat abcd abcd abcd abcd abcd surprisingly, a simple low-level hack made a tremendous difference: when constructing the sprite I compressed blocks of 32 1-bit pixels into 32-bit integers, thus reducing the number of checks (and memory) by 32 times.")[:30], maxsize=120, colors=COLOR_SCHEMES['audacity'])
